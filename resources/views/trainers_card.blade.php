        ...
        <div class="grid grid-cols-3 mb-8" id="courses_box">
            <div class="relative col-span-1">
                ...
                <div class="mx-2 lg:mx-6 translate-y-1/4">
                    ...
                    @else
                        <img src="{{ asset($trainers->image) }}" alt="" class="block mx-auto rounded-full profile_pic" style="min-width: 60px">
                    @endif
                </div>
            </div>
            <div class="relative col-span-2">
                <div class="mx-6 mt-8 lg:text-xl f_gay" >
                    <h1 class="font-semibold lg:text-4xl">{{ Str::limit($trainers->tfname . " " . $trainers->tlname, 20) }}</h1>
                </div>
                <div class="mx-6 mt-2 lg:text-xl f_gray">
                    <h1 class="font-semibold lg:text-4xl">{{ "#" . $trainers->trainerid }}</h1>
                </div>
                <div class="mx-6 mt-5 lg:text-xl f_gray">
                    <h2 class="lg:text-2xl"><span class="font-medium">ที่อยู่&nbsp;</span> {{ $trainers -> address }} </h2>
                </div>
                <div class="mx-6 mt-3 lg:text-xl f_gray">
                    <h2 class="lg:text-2xl"><span class="font-medium">เบอร์โทรศัพท์&nbsp;</span> {{ $trainers -> phone }} </h2>
                </div>
                <div class="flex justify-center my-4">
                    {!! DNS1D::getBarcodeHTML("$trainers->trainerid", 'PHARMA', 5, 60) !!}
                 </div>
            </div>
        </div>
    </div>
