...
@section('content')
    ...
    <div class="flex flex-col items-center justify-center section">
        ...
        <div class="w-3/4 mt-2 flex_items">
            <h1 class="text-3xl font-semibold text-center f_gray xs:text-4xl sm:text-5xl lg:text-6xl">เข้าสู่ระบบผู้ฝึกสอน</h1>
        </div>

        <div class="w-3/4 mt-2 flex_items md:mt-6">
            <form method="POST" action="{{ route('loginPost.Trainers') }}" name="login">
                @csrf
                ...
                <div class="pl-3 mx-auto mt-2" style="max-width: 450px">
                    <a href="{{ route('forgot.Trainers') }}" class="z-0 mr-2 text-sm underline opacity-75 f_gray sm:mr-4 md:text-base lg:text-lg">ลืมรหัสผ่าน?</a>
                </div>
        ...
        <div class="w-3/4 mt-12 text-center flex_items md:mt-16">
            <a href="{{ route('register.Trainers') }}" class="ml-1 text-xs underline f_gray sm:text-sm md:text-base lg:text-lg">สมัครเป็นผู้ฝึกสอน</a>
        </div>
    </div>
