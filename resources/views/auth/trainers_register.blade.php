...
@section('content')
    ...
    <div class="flex flex-col items-center justify-center mt-3 mb-14 sm:mt-10 xl:mt-0 xl:mb-0 section">
        ...
        <div class="w-3/4 xl:mb-4 flex_items">
            <h1 class="text-3xl font-semibold text-center f_gray xs:text-4xl sm:text-5xl lg:text-6xl">สมัครเป็นผู้ฝึกสอน</h1>
        </div>

        <div class="w-3/4 mt-2 flex_items md:mt-6" style="max-width: 1366px">
            <form method="POST" action="{{ route('registerPost.Trainers') }}" class="">
                @csrf
                <div class="grid grid-cols-7 xl:gap-4">
                    ...
                    <div class="relative w-full col-span-7 mx-auto mt-3 md:mt-6 xl:col-span-1 xl:mt-0" style="max-width: 450px">
                        <select name="sex" id="sex" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-5 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray ">
                    ...
                    <!-- experience -->
                    <div class="relative w-full col-span-7 mx-auto mt-3 md:mt-6 xl:col-span-2 username xl:mt-0">
                        <input id="experience" type="number" name="experience" placeholder=" " value="@if(session('error')){{ session('error')[9] }}@endif" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-5 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                        <label for="experience" class="absolute block pl-3 mx-auto pointer-events-none f_gray md:text-lg">ประสบการณ์การสอน (3 ปีขึ้นไป)</label>
                        <!-- error Xl -->
                        @error('experience')
                            <div class="absolute hidden w-full col-span-3 mx-auto xl:block top-3/4 left-3 ">
                                <p class="block pl-3 text-xs pointer-events-none f_red">{{ $message }}</p>
                    ...
                    <!-- Email -->
                    <div class="relative w-full col-span-7 mx-auto mt-3 md:mt-6 xl:col-span-2 email xl:mt-0">
                        <input id="email" type="email" name="email" placeholder=" " value="@if(session('error')){{ session('error')[10] }}@endif" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-5 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                    ...
                    <div class="relative w-full col-span-7 mx-auto mt-3 md:mt-6 xl:col-span-3 password xl:mt-0">
                        <input id="password" type="password" name="password" placeholder=" " value="@if(session('error')){{ session('error')[11] }}@endif" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-5 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                    ...
                    <!-- Register Button -->
                    <div class="flex justify-center w-full col-span-7 mt-4 ml-auto jus md:mt-10 xl:justify-end register_btt xl:mt-0" style="max-width: 576.3px">
                        <div class="grid w-full grid-cols-1 mx-auto sm:block sm:mx-0">
                            <button type="submit" id="login" class="w-full p-4 mx-auto mt-4 text-white rounded-2xl xs:rounded-3xl md:text-xl md:p-5 lg:text-2xl" style="min-width: 128px">สมัครเป็นผู้ฝึกสอน</button>
                        </div>
                    </div>
                </div>
            </form>
