...
@section('content')
    ...
    <div class="flex flex-col items-center justify-center section">
        ...
        <div class="w-3/4 mt-2 flex_items md:mt-6">
            <form method="POST" action="{{ route('resetPasswordPost.Members') }}" name="login">
                @csrf
                ...
                <div class="relative mx-auto mt-2 md:mt-6" style="min-width: 250px; max-width: 450px">
                    <input id="email" type="email" name="email" placeholder=" " value="@if (session('error')){{ session('error')[0] }}@endif" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-4 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                ...
                <div class="relative mx-auto mt-3 xs:mt-4 md:mt-6" style="min-width: 250px; max-width: 450px">
                    <input id="password" type="password" name="password" placeholder=" " value="@if (session('error')){{ session('error')[1] }}@endif" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-4 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                    <label for="password" class="absolute block pl-3 mx-auto pointer-events-none f_gray md:text-lg">รหัสผ่าน</label>
                </div>

                <!-- confirm_password -->
                <div class="relative mx-auto mt-3 xs:mt-4 md:mt-6" style="min-width: 250px; max-width: 450px">
                    <input id="password_confirmation" type="password" name="password_confirmation" placeholder=" " value="@if (session('error')){{ session('error')[2] }}@endif" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-4 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                ...
                @if (session()->has('error'))
                    <div class="w-full col-span-7 mx-auto mt-1" style="min-width: 250px; max-width: 450px">
                        <p class="block pl-3 text-sm pointer-events-none f_red">{{ session('error') }}</p>
                    </div>
                @endif
