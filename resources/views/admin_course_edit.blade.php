...
@section('content')
    ...
    <div class="flex flex-col items-center justify-center pt-10 mb-20 lg:pl-48">
        ...
        <div class="w-3/4 mt-2 flex_items md:mt-6 display_content" style="max-width: 900px">
            ...
            <form method="POST" action="{{ route('editCoursesPost.Admin', ["courseid" => $courses->courseid, "exerciseid" => $courses->exerciseid, "dietid" =>$courses->dietid]) }}" class="">
                @csrf
                <div class="grid grid-cols-2 xl:grid-cols-2 lg:gap-4">
                    ...
                    <div class="relative w-full col-span-2 mx-auto mt-3 lg:col-span-1 xl:mt-0" style="max-width: 450px">
                        <input id="cname" type="text" name="cname" placeholder=" " value="{{ $courses -> cname }}" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-5 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                        <label for="cname" class="absolute block pl-3 mx-auto pointer-events-none f_gray md:text-lg">ชื่อคอร์ส</label>
                    </div>

                    <!-- Course Price -->
                    <div class="relative w-full col-span-2 mx-auto mt-3 lg:col-span-1 xl:mt-0" style="max-width: 450px">
                        <input id="cost" type="number" name="cost" placeholder=" " value="{{ $courses -> cost }}" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-5 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                        <label for="cost" class="absolute block pl-3 mx-auto pointer-events-none f_gray md:text-lg">ราคา</label>
                    </div>

                    <!-- Course Day -->
                    <div class="relative w-full col-span-2 mx-auto mt-3 lg:col-span-1 xl:mt-0" style="max-width: 450px">
                        <input id="course_day" type="number" name="course_day" placeholder=" " value="{{ $courses -> course_day }}" class="block w-full h-full px-4 pt-6 pb-2 mx-auto text-sm rounded-xl sm:rounded-2xl sm:px-5 sm:text-sm md:px-6 md:text-base md:pt-7 md:pb-4 lg:text-base f_gray "/>
                    ...
                    <script>
                        var valueToSelect = "{{ $dietid }}";
                        var selectElement = document.getElementById('dietid');
                ...
                <div class="flex justify-center w-full col-span-2 mx-auto mt-4 lg:ml-auto lg:mx-0 xl:justify-end add_btt lg:mt-4">
                    <div class="grid w-full grid-cols-1 sm:block sm:mx-0">
                        <button type="submit" id="editcourse" class="w-full p-4 mx-auto mt-4 text-white rounded-2xl xs:rounded-3xl md:text-xl md:p-5 lg:text-2xl" style="min-width: 128px">ยืนยัน</button>
                    </div>
                </div>
            </form>
        </div>
