        ...
        <div class="grid grid-cols-5" id="col_head" style="max-width: 1200px">
            ...
            @foreach ($membercards as $membercard)
                ...
                <div class="relative col-span-2 px-6 py-3 text-center f_gray row-col lg:text-2xl">
                    {{ \Carbon\Carbon::parse($membercard->checkin)->addYears(543)->isoFormat('วันที่ D เดือน MMMM ปี YYYY เวลา H:mm', 'th') }}
                </div>
                <div class="relative col-span-2 px-6 py-3 text-center f_gray row-col lg:text-2xl">
                    @if ($membercard->checkout == null)
                        กำลังออกกำลังกาย
                    @else
                    {{ \Carbon\Carbon::parse($membercard->checkout)->addYears(543)->isoFormat('วันที่ D เดือน MMMM ปี YYYY เวลา H:mm', 'th') }}
                    @endif
            ...
            @endphp
            @endforeach
            <a href="{{ route('testCheckin.Members', [ "memberid" => $members -> memberid, "fullname" => $members -> mfname . " " . $members -> mlname])}}" class="underline">test checkin</a>
            <a href="{{ route('testCheckout.Members', $members -> memberid ) }}" class="underline">test checkout</a>
        </div>
    </div>
@endsection
